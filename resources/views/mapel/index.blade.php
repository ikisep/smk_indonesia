...
<div class="bg-white shadow-md rounded-lg p-6">
    <div class="flex justify-between items-center border-b pb-4 mb-4">
        ...
        <a href="{{ route('mapel.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600">
            ➕ Tambah Mapel
        </a>
    </div>
            ...
            <tbody class="text-gray-700">
                @foreach ($mapels as $mapel)
                    <tr class="border-b hover:bg-gray-100">
                        <td class="px-4 py-2 font-semibold">{{ $mapel->id }}</td>
                        <td class="px-4 py-2">{{ $mapel->nama }}</td>
                        <td class="px-4 py-2 text-center flex space-x-2 justify-center">
                            <a href="{{ route('mapel.edit', $mapel->id) }}" class="bg-green-500 text-white px-3 py-1 rounded-lg hover:bg-green-600">
                                ✏️ Edit
                            </a>
                            <form action="{{ route('guru.destroy', $mapel->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
